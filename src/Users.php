...
class Users extends Conexion
{
    ...
    {
        $q = "update users set username=:u, email=:e, password=:pa, perfil=:pe, img=:i where id=:id";
        $stmt = parent::$conexion->prepare($q);
        try {
            $stmt->execute([
                ':u' => $this->username,
    ...
    {
        $q = (isset($this->id)) ? "select * from users where $campo=:v AND id!={$this->id}" : "select * from users where $campo=:v";
        $stmt = parent::$conexion->prepare($q);
        try {
            $stmt->execute([
                ':v' => $valor
